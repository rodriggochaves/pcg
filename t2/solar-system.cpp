...
GLuint myInit_IlumTexture(GLFWwindow* fenetre, const char* vs, const char* fs)
{
  ...
  color4 light_ambient(  0.8, 0.8, 0.8, 1.0 );
...
void myDisplaySun ( GLuint sp4 )
{
  ...
  model = Scale(.65) * RotateY(-1.5*angulo);
...
void myDisplayPlanet ( GLuint sp4 )
{
  ...
  model = Scale(.35) * RotateY(-1.5*angulo);
  // view = Translate(0, 0, -20);
  view = RotateY(2 * angulo) * Translate(7, 0, 0);
...
void myDisplayMoon ( GLuint sp4 , GLfloat anguloLua)
{
	glfwMakeContextCurrent(window);
	...
	model = Scale(.15) * RotateY(-1.5 * angulo);
	// view = Translate(0, 0, -20);
	view = RotateY(2 * angulo) * Translate(7, 0, 0) * RotateY(2 * angulo) * Translate(2, 0, 0);
	projection = Ortho(-10,10,-10,10,-10,10);
